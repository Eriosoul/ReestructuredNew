// frontend/components/EditUnitForm.tsx
...
import { Ship, Plane, Truck, Crosshair, Loader2, Plus, X } from 'lucide-react';
...
export function EditUnitForm({ unitId, onSuccess, onCancel, entityType = 'units' }: EditUnitFormProps) {
  ...
  // Estado del formulario (similar al de creación)
  ...
  useEffect(() => {
    const fetchUnit = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/${entityType}/${unitId}`);
        if (!res.ok) throw new Error('Error al cargar unidad');
        const data = await res.json();

        // Mapear los datos recibidos al estado del formulario
        setFormData({
          _id: data._id,
          unitId: data.unitId || '',
          name: data.name || '',
          ...
          images: Array.isArray(data.images) ? data.images.map((img: any) => typeof img === 'string' ? img : img.url) : [],
          videos: Array.isArray(data.videos) ? data.videos.map((vid: any) => typeof vid === 'string' ? vid : vid.url) : [],
          ...
          metadata: data.metadata || { sources: ['Usuario'], dataQuality: 'media' },
        });

        setDomain(data.domain || 'naval');
      } catch (error) {
    ...
    fetchUnit();
  }, [unitId, entityType, API_BASE_URL, onCancel]);
  ...
  // Subida de archivos (igual que en CreateUnitForm)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/${entityType}/${unitId}`, {
        method: 'PUT',
  ...
  // Renderizadores técnicos (idénticos a los de CreateUnitForm)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 p-2">
      ...
      <div className="flex justify-end gap-4 pt-6 border-t">
        <Button type="button" variant="outline" onClick={onCancel} disabled={saving} className="h-12 px-6 text-base">
          Cancelar
        </Button>
