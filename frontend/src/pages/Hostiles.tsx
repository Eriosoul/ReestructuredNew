// frontend/pages/HostilePage.tsx
...
export default function HostilePage() {
  ...
  const fetchUnits = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/hostile`);
  ...
  const fetchUnitMissions = async (unitId: string) => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/hostile/${unitId}/missions`);
  ...
  const handleDelete = async (unitId: string, unitName: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/hostile/${unitId}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Catálogo de Hostiles o Sospechosos</h1>
